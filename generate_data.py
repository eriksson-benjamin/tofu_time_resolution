...
time_dict = dfs.get_dictionaries('merged')
erg_dict = dfs.get_dictionaries('merged')
...
for directory in directories:
    ...
    for boa in boards:
        for cha in channels:
            ...
            # Import pulses/times
            pulse_data = udfs.import_pulses(board=boa, channel=cha, path=path,
            ...
            if sx not in ['ABS_REF', '1kHz_CLK']:
                pulse_min = np.min(pulse_data, axis=1)
                clipped_args = np.where(pulse_min > -0.95 * bias)[0]
            ...
            know which trigger level was used we can find the number of pre
            trigger samples.
            '''
            ...
            # Baseline reduction
            pulse_data_bl = dfs.baseline_reduction(pulse_data,
                                                   timer=timer_level)
    ...
    to_pickle = {'time': time_dict, 'energy': erg_dict, 'info': info}
    udfs.pickler(f'{directory}.pickle', to_pickle)
